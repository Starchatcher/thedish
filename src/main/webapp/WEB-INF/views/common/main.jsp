...
<section class="hero">
    ...
    <c:choose>
        <c:when test="${not empty randomRecipe and randomRecipe.recipeId != 0}">
            <a href="recipeDetail.do?no=${randomRecipe.recipeId}">
                <c:choose>
                    <c:when test="${not empty randomRecipeImage.imageUrl}">
                        <img src="${randomRecipeImage.imageUrl}" alt="${randomRecipe.name} 이미지" class="recommended-image"/>
                    </c:when>
                    <c:when test="${not empty randomRecipeImage.imageId and randomRecipeImage.imageId != 0}">
                        <img src="${pageContext.request.contextPath}/image/view.do?imageId=${randomRecipeImage.imageId}" alt="${randomRecipe.name} 이미지" class="recommended-image"/>
                    </c:when>
                    <c:otherwise>
                        <img src="<c:url value='/resources/images/default-image.png'/>" alt="이미지 없음" class="recommended-image"/>
                    </c:otherwise>
                </c:choose>
            </a>
        ...
        <c:otherwise>
            <img src="<c:url value='/resources/images/default-image.png'/>" alt="추천 레시피 이미지 없음" class="recommended-image"/>
            <h3>추천 레시피를 찾을 수 없습니다.</h3>
            <p>로그인 후 다시 시도하거나 나중에 확인해주세요.</p>
        </c:otherwise>
    </c:choose>
</section>
